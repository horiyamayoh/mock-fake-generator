...
| 領域 | 採用候補 ket module | 適用内容 | 備考 |
|---|---|---|---|
| CLI parse | `ket::cli`, `ket::parse` | `--input-root`、`--output-dir`、`--jobs` などの取得と数値変換 | `ket::cli` の簡易 option semantics をツール仕様として固定する |
| ファイル I/O | `ket::file` | config、report、manifest、生成物の text read/write | recursive traversal は `std::filesystem` を直接使う |
| 文字列補助 | `ket::ascii`, `ket::string` | prefix/suffix、trim、診断文・ファイル名 fragment の構築 | AST 型表記や C++ signature 復元には使わない |
| RAII cleanup | `ket::scope` | 一時ディレクトリ削除、失敗時 cleanup、状態復元 | Clang state の lifetime 管理を明示する |
| 契約チェック | `ket::contract` | 内部 invariant、precondition、到達不能な状態の検出 | ユーザー入力エラーには使わず、通常診断として返す |
| 並行補助 | `ket::concurrency` | 必要な場合の join 漏れ防止、future ready 判定 | thread pool framework としては使わない |
| テスト補助 | `ket::testing` | 生成物 golden 差分の bytes/hex 表示に任意利用 | gTest 依存を test helper に閉じ込める |

`ket::format`、`ket::container` など未導入または未実装の module は、ツール初期版の必須依存にしない。利用する場合は、対象 ket module が実装済みであり、drop-in できることを確認してから採用する。
...
推奨は、ket を repository submodule ではなく **必要 module だけ vendoring する** 方式である。
...
1. 使う ket module だけを `third_party/ket/modules/<name>/` にコピーする。
2. `KET_COMMIT.txt` に取得元 commit hash と取得日を記録する。
3. ket module 自体は原則として改変しない。
4. 必要な workaround は `src/support/KetCompat.h` または `src/support/KetSupport.*` に閉じ込める。
5. ket module 追加時は CMake、ライセンス表記、CI、`docs/dependency_inventory.md` を同時に更新する。
6. 生成物 compile fixture には ket include directory を渡さず、生成物が ket なしでコンパイルできることを検証する。
...
src/support/FileSystem.*     -> ket::file, ket::ascii を使用可
src/support/Text.*           -> ket::string, ket::ascii を使用可
src/support/Scope.*          -> ket::scope を使用可
src/support/Contract.*       -> ket::contract を使用可
src/support/Threading.*      -> ket::concurrency を使用可
...
例外的に `generator` が `ket::string::Cat` 程度の純粋文字列補助を使うことは許可するが、AST 意味解析・型表記・生成物 template には ket を混ぜない。
...
`OutputWriter` は `ket::file::TryWriteAllText` を基本に使う。ただし、atomic replace が必要な場合は標準 `std::filesystem` と POSIX `rename` 相当を使い、`ket::file` の非 atomic write 仕様に依存しすぎない。
...
| `compile-error` | 未登録 fallback が必要な関数を意図的に compile error にする | 完全な期待設定を強制する厳格モード |
...
| FileSupport     ket::file   |
| TextSupport     ket::ascii  |
| ScopeSupport    ket::scope  |
| ContractSupport ket::contract|
+-----------------------------+
```
...
`--input-root` 配下を `std::filesystem::recursive_directory_iterator` で探索し、通常ファイルかつ拡張子 `.h` のものを対象候補とする。path traversal 自体は標準 `std::filesystem` を直接使い、拡張子・除外 pattern・診断用 path 表記の軽量文字列処理に限って `ket::ascii` を使ってよい。
...
- `ket::contract` は `Config` の内部 invariant 用に限る。
...
- `ket::ascii` による suffix/prefix 判定と診断用 path 表記補助

注意:

- filesystem traversal は `std::filesystem` を直接使う。
- ket は path 操作ライブラリとしてではなく、文字列補助としてのみ使う。
...
- `CliSupport`: `ket::cli` を薄く包み、Config 以外から CLI details を隠す
- `FileSupport`: `ket::file` と `std::filesystem` の使い分けを集約する
- `TextSupport`: `ket::ascii` / `ket::string` の利用箇所を集約する
- `ScopeSupport`: `ket::scope` による cleanup helper を提供する
- `ContractSupport`: `ket::contract` を使った内部 invariant helper を提供する
...
ツール本体は C++23 固定でビルドする。ket は必要 module だけを `third_party/ket/modules` に vendoring し、専用 static library `mockfakegen_ket` としてリンクする。
...
add_library(mockfakegen_ket STATIC
    "${KET_MODULE_DIR}/file/ket_file.cpp"
    "${KET_MODULE_DIR}/ascii/ket_ascii.cpp"
)

target_include_directories(mockfakegen_ket PUBLIC
    "${KET_MODULE_DIR}/cli"
    "${KET_MODULE_DIR}/file"
    "${KET_MODULE_DIR}/ascii"
    "${KET_MODULE_DIR}/string"
    "${KET_MODULE_DIR}/parse"
    "${KET_MODULE_DIR}/scope"
    "${KET_MODULE_DIR}/contract"
    "${KET_MODULE_DIR}/concurrency"
)
...
add_executable(mockfakegen
    ...
    src/support/FileSupport.cpp
    src/support/TextSupport.cpp
    src/support/ScopeSupport.cpp
    src/support/ContractSupport.cpp
)
...
### 25.1 ket vendoring の CMake ルール

- ket の include directory を `mockfakegen_ket` の `PUBLIC` include directory に閉じる。
...
- ket module 追加時は `third_party/ket/KET_COMMIT.txt` と `docs/dependency_inventory.md` を更新する。
...
- `third_party/ket` の最小 vendoring
- `ket::cli` を使った `--help` / 必須 option parse
- `ket::file` を使った単一ファイル出力
...
- ツール本体は vendored ket module を使用してビルドされる。
...
- report writer
- ket-backed `CliSupport` / `FileSupport` / `TextSupport`
...
1. `third_party/ket` の最小 vendoring と CMake skeleton
2. `Config` / `CliSupport`
3. `HeaderScanner`
